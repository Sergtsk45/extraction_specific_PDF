...
description: Task tracking and plan management. Used by planner to persist tasks and by orchestrator to track progress.
---
...
- **Planner** creates plans + detailed tasks
- **Orchestrator** reads tasks, updates progress
- **Documenter** creates completion reports
- **Any agent** can create issues for non-critical problems
...
for (taskId of taskIds) {
  ...
  // Run tests & verify (combined in test-runner now)
