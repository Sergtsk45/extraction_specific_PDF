...
> **Дата:** 2026-03-01  
> **Статус:** Два микросервиса реализованы, интегрированы и настроены на автозапуск через systemd
...
**Автозапуск (рекомендуется, WSL2 + systemd):**

Все сервисы зарегистрированы как systemd user-units и стартуют автоматически при загрузке WSL2:
...
Unit-файлы: `~/.config/systemd/user/{invoice-extractor,spec-converterv2,docplatform-dev}.service`
...
│   ├── uploads/                 # Временная папка для загруженных PDF
│   └── outputs/                 # Папка для сгенерированных XLSX
├── README.md
└── start.sh
```
