...
### 2.4 Текущее состояние репозитория (важно)

- **Активные сервисы**: `spec-converterv2` (порт 5001), `invoice-extractor` (порт 5002).
- **Planned сервисы**: `ocr`, `docx-merger`, `project-validator` сейчас представлены **только `manifest.json`** (статус `"planned"`), без `backend/` и без `component.js`.
- **Dev-прокси**: локальная разработка обычно идёт через `dev_server.py` (порт 8080), который проксирует API активных сервисов.
- **Production-шаблоны**: `gateway/nginx.conf` и unit-файлы systemd лежат в `gateway/systemd/` как готовые конфиги/шаблоны.
...
### 3.5 Локальная разработка (без Nginx)

Для разработки оболочки и микросервисов без Gateway/Nginx используется `dev_server.py`.

**Автозапуск (WSL2 + systemd):**

Автозапуск может быть настроен через systemd. В репозитории хранятся готовые unit-файлы в `gateway/systemd/` (их нужно установить в систему вручную). Ранее использовались user-units в `~/.config/systemd/user/`, но источником truth для деплоя считаем unit-файлы из репозитория.

```bash
# Статус
systemctl --user status invoice-extractor spec-converterv2 docplatform-dev

# Перезапуск
systemctl --user restart invoice-extractor

# Просмотр логов
journalctl --user -u invoice-extractor -f
```

Unit-файлы в репозитории: `gateway/systemd/{invoice-extractor,spec-converterv2,nginx}.service`

**Ручной запуск (альтернатива):**

- Shell dev-server (порт **8080**):
  - `cd project-root/ && python3 dev_server.py`
- Бэкенд `spec-converterv2` (порт **5001**):
  - `cd services/spec-converterv2/backend && python3 app.py`
- Бэкенд `invoice-extractor` (порт **5002**):
  - `cd services/invoice-extractor/backend && python3 app.py`

**Что делает dev-server:**

- **Редирект**: `GET /` → `302` → `/shell/index.html` (важно для корректной загрузки относительных `css/js`).
- **Прокси**: `/api/spec-converter/*` → `http://127.0.0.1:5001/*`.
- **Прокси**: `/api/invoice-extractor/*` → `http://127.0.0.1:5002/*`.

**Типовые проблемы и решения:**

- **Shell “сломался” (видно модалку/дропзоны, нет стилей)**: обычно это 404 на `/css/...` и `/js/...` из-за открытия корня без редиректа. В dev используется редирект на `/shell/index.html`, чтобы относительные пути работали.
- **HTTP 502 при конвертации**: может возникать при падении бэкенда. Ранее причина была в кириллице в HTTP-заголовке `X-Sheet-Names` (заголовки должны быть ASCII/latin-1). Решение: URL-encoding на бэкенде и `decodeURIComponent` на фронтенде.
- **Чипы листов отображаются как `%D0%...`**: это URL-encoded строка. Фронтенд декодирует её перед рендером.
- **Правки `component.js` не подхватываются**: браузер может кэшировать ES-модули (в т.ч. при 304). В dev на localhost используется cache-busting query при динамическом `import()` компонентов, чтобы подтягивать свежую версию.
...
## 6. Микросервис: invoice-extractor (внедрённый)

### 6.1 Назначение

Парсинг PDF-счетов поставщиков с автоматическим извлечением структурированных данных (счет, дата, поставщик, товары, стоимости) в Excel-файл (.xlsx) с поддержкой LLM Vision для сложных или отсканированных документов.
...
|------|-----------|------------|
| Frontend | HTML/CSS/JS (статика) | Drag-and-drop, вызов API |
| Backend | Python 3.11+, Flask 3.0 | REST API, парсинг и экспорт |
| PDF → Data | pdfplumber + PyMuPDF | Извлечение текста и рендеринг |
| Vision (LLM) | Anthropic / OpenRouter / OpenAI | Распознавание данных из сканов |
| Excel | openpyxl | Генерация .xlsx |
| Валидация | Custom validators | Проверка качества парсинга |

### 6.3 Поток данных

```mermaid
flowchart LR
    A["PDF счета"] --> B{"Текстовый слой?"}
    B -->|Да| C["pdfplumber<br/>Извлечение текста"]
    B -->|Нет| D["PyMuPDF → Image<br/>→ LLM Vision"]
    C --> E["Нормализация<br/>invoice_data"]
    D --> E
    E --> F["Валидация"]
    F --> G["openpyxl<br/>Excel Builder"]
    G --> H[".xlsx файл"]
```
...
- `X-Document-Type` — тип документа (invoice)
- `X-Parse-Quality` — качество парсинга (ok / partial)
- `X-Job-Id` — идентификатор обработки
- `X-Invoice-Number` — номер счета (URL-encoded)

### 6.5 Ключевые особенности

- **Vision-режим по умолчанию отключен** — используется text-first пайплайн для скорости и экономии.
...
│   └── index.html               # Standalone-режим (для автономного использования)
├── backend/
│   ├── app.py                   # Flask: /health, /convert, основной API
│   ├── app/
│   │   ├── __init__.py
...
│   ├── uploads/                 # Временная папка для загруженных PDF (по умолчанию)
│   └── outputs/                 # Папка для сгенерированных XLSX (по умолчанию)
├── README.md
└── start.sh
```
...
## 7. Технологический стек платформы

## 7. Технологический стек платформы

### 7.1 Оболочка (Shell) — утверждённый стек
...
### 7.2 Стандарты для микросервисов
...
**Примечание по манифестам (текущее состояние):**
- `maxFileSize` в разных сервисах может быть **числом (байты)** или строкой вида `"50MB"`. Shell должен уметь оба варианта.

**Примечание по файловым директориям (uploads/outputs):**
- `spec-converterv2` хранит временные файлы и результаты **в каталогах на уровне сервиса** (путь относительно `services/spec-converterv2/`): `uploads/` и `output/` (singular).
- `invoice-extractor` по умолчанию использует каталоги **внутри `backend/`**: `backend/uploads/` и `backend/outputs/` (plural). Пути могут быть переопределены через переменные окружения `UPLOAD_FOLDER` и `OUTPUT_FOLDER`.
- При деплое важно учитывать различие `output` vs `outputs` и не смешивать их между сервисами (особенно при настройке volume’ов/cleanup).

---

## 8. Безопасность

### 8.1 Текущие меры (spec-converterv2, invoice-extractor)
...
### 8.2 Планируемые меры (платформа)
...
### 9.1 Текущие характеристики (spec-converterv2, invoice-extractor)
...
### 9.2 Планируемые оптимизации (по этапам)
...
│   ├── spec-converterv2/              # Микросервис 1 (спецификации → Excel)
│   │   ├── component.js               # Web Component для встраивания в Shell
│   │   ├── manifest.json              # Манифест для регистрации в оболочке
│   │   ├── Dockerfile
│   │   ├── frontend/
...
│   │   └── ...
│   │
│   ├── invoice-extractor/             # Микросервис 2 (счета → Excel)
│   │   ├── component.js               # Web Component для встраивания в Shell
│   │   ├── manifest.json              # Манифест для регистрации в оболочке
│   │   ├── Dockerfile
│   │   ├── frontend/
...
│   │   ├── backend/
│   │   │   ├── .env                   # API-ключи (не в репо)
│   │   │   ├── app/                   # Основной парсер и экспортер
│   │   │   └── ...
│   │   ├── README.md
│   │   └── ...
│   │
...
**Важно:** `docker-compose.yml` описан как целевая цель, но в текущем репозитории может отсутствовать. Для запуска в production используйте `gateway/nginx.conf` + systemd unit-файлы из `gateway/systemd/`, либо добавьте compose-файл отдельной задачей.

---

## 12. Соглашения и стандарты
